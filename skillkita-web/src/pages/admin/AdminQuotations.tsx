  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-2xl text-sm text-black">
        Review employer submissions. Set company name/address (for the PDF), mode, unit price, and total amount (RM),
      ...
      {activeReview && activeReview.status === "pending" && (
        <section className="sk-card mt-8 p-6">
          ...
          <div className="mt-4 rounded-lg border border-[#efe1db] bg-[#faf7f2] p-4 text-sm">
            ...
            <p className="mt-1">
              <span className="font-semibold">Proposed booking date:</span> {activeReview.proposed_date}
            </p>
      ...
      <section className="sk-card mt-8 p-6">
        ...
        {!isLoading && rows.length === 0 && <p className="mt-4 text-sm text-black">No quotation requests yet.</p>}
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-[#efe1db]">
                    <td className="py-3 pr-3 align-top">{getEmployerDisplayName(r)}</td>
