...
const ViewCourses = () => {
  ...
  const body = (
    <main className="sk-container py-12">
      <h1 className="text-4xl font-bold text-[#0001fc] md:text-5xl">Available Courses</h1>
      <p className="mt-3 text-lg text-black md:text-xl">
        Browse training programs that is currently available. Log in to contact us for more information
          ...
          {!isLoading && (
            <ul className="mt-4 space-y-4">
              {employerCourses.map((c) => {
                ...
                return (
                  <li key={c.id} className="rounded-xl border border-[#efe1db] bg-white p-4 shadow-sm">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 border-t border-[#efe1db] pt-4">
                        {(Object.keys(PRIVATE_DOC_LABELS) as PrivateDocKind[]).map((kind) => {
                          ...
                          return (
                            ...
                            >
                              {path
                                ? `Open ${PRIVATE_DOC_LABELS[kind]}`
                                : `${PRIVATE_DOC_LABELS[kind]} (n/a)`}
                            </button>
                          );
                        })}
                      </div>
